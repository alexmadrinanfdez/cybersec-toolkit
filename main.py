...
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == '0':
            ...
            print("Hash value:", hash_module.hash_file(file_path))
        elif choice == '2':
            orig_path = input("Enter the original file path: ")
            dest_path = input("Enter the file path to check: ")
            check = hash_module.verify_integrity(orig_path, dest_path)
            print("Integrity check: files", "are identical." if check else "differ.")
        elif choice == '3':
            message = input("Enter the message to encrypt: ")
            key = encrypt_module.generate_key("AES")
            ciphertext = encrypt_module.aes_encrypt(message, key)
            print("Encrypted message:", ciphertext.hex())
            print("Encryption key:", key.hex())
        elif choice == '4':
            ciphertext = input("Enter the ciphertext to decrypt: ")
            key = input("Enter the encryption key: ")
            plaintext = encrypt_module.aes_decrypt(bytes.fromhex(ciphertext), bytes.fromhex(key))
            print("Decrypted message:", plaintext)
        elif choice == '5':
            password = getpass("Enter the password to check: ", echo_char='*')
            score, feedback = pw_module.check_password_strength(password)
            print(f"Password strength: {score}/4")
            print(pw_module.parse_feedback(feedback))
        else:
